...
class Airplane {
 ...
 class Car {
   constructor(model, milesPerGallon) {
    ...
    drive(distance) {
      if ((this.tank - distance / this.milesPerGallon) <= 0) {//determines if tank will run out of gas
        const x = this.tank * this.milesPerGallon;// x is maximum amount car can travel until running out of gas
        this.odometer = this.odometer + x;
        this.tank = 0;
        return `I ran out of fuel at ${this.odometer} miles!`;
      } else {//executes if car will not reach zero gallons by end of distance traveled
 ...
 class Instructor extends Lambdasian{
   ...
   grade(student, subject) {
     return `${student.name} receibes a perfect score on ${subject}`;
   }
 }
  /*
    TASK 5
